...
class Scraper():
    def __init__(self):
        ...
        self.schedule = self.client.schedule.get_schedule(date="2024-11-22")
        
        def get_matchup(i):
            # json path to extract matchups
            matchup_json_path = self.schedule.get('games')[i].get('awayTeam').get('placeName')['default']
            return matchup_json_path
